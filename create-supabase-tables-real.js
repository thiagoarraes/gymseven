// Script para criar tabelas no Supabase real usando a URL correta
import pg from 'pg';
const { Client } = pg;

// Construir a URL do Supabase corretamente
...
// Extrair o projeto ID da URL (ex: vlqzjrwxqeyroqsglqwr)
const projectId = supabaseUrl.match(/https:\/\/([^.]+)\.supabase\.co/)[1];

// Construir URL de conexão PostgreSQL para Supabase
const supabaseDbUrl = `postgresql://postgres:${supabaseKey.split('.')[1] || 'password'}@db.${projectId}.supabase.co:5432/postgres`;

console.log('🔗 Usando URL do Supabase...');
...
async function createTables() {
  try {
    ...
    console.log('📋 Tabelas existentes no Supabase:', checkResult.rows.map(r => r.table_name));
    ...
    console.log('\n🔍 Agora verifique seu painel do Supabase:');
