...
async function checkUserIsolation() {
  ...
  try {
    ...
    exercises?.forEach((ex, i) => {
      console.log(`  ${i + 1}. ${ex.name} | UserID: ${ex.userId || 'SEM USER_ID'} | ID: ${ex.id.slice(0, 8)}...`);
    });
    ...
    templates?.forEach((tmpl, i) => {
      console.log(`  ${i + 1}. ${tmpl.name} | UserID: ${tmpl.userId || 'SEM USER_ID'} | ID: ${tmpl.id.slice(0, 8)}...`);
    });
    
    // Check workout logs
    const { data: logs, error: logsError } = await supabase
      .from('workoutLogs')
      .select('id, name, userId, startTime')
    ...
    logs?.forEach((log, i) => {
      console.log(`  ${i + 1}. ${log.name} | UserID: ${log.userId || 'SEM USER_ID'} | ID: ${log.id.slice(0, 8)}...`);
    });
    
    // Check unique users with data
    const uniqueUserIds = new Set([
      ...(exercises?.map(e => e.userId).filter(Boolean) || []),
      ...(templates?.map(t => t.userId).filter(Boolean) || []),
      ...(logs?.map(l => l.userId).filter(Boolean) || [])
    ]);
    
    console.log('\n👥 RESUMO:');
    console.log(`Usuários únicos com dados: ${uniqueUserIds.size}`);
    console.log(`Exercícios sem userID: ${exercises?.filter(e => !e.userId).length || 0}`);
    console.log(`Templates sem userID: ${templates?.filter(t => !t.userId).length || 0}`);
    console.log(`Logs sem userID: ${logs?.filter(l => !l.userId).length || 0}`);
